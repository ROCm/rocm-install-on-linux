...
setup_repo_priorities() {
    ...
    if [ -f /etc/yum.repos.d/epel.repo ]; then
        if ! $SUDO grep -q "excludepkgs=rocm" /etc/yum.repos.d/epel.repo; then
            echo "Excluding ROCm and HIP packages from EPEL..."
            $SUDO sed -i '/\[epel\]/a excludepkgs=rocm* hip*' /etc/yum.repos.d/epel.repo
        fi
        
        if ! $SUDO grep -q "priority=" /etc/yum.repos.d/epel.repo; then
    ...
    if [ -f /etc/yum.repos.d/rocm.repo ]; then
        if ! $SUDO grep -q "includepkgs=rocm" /etc/yum.repos.d/rocm.repo; then
...
setup_installer_public_repos() {
    ...
    # amdgpu and graphics URL for rhel 8.x and 10.x only contains the major version of the distro for ROCm 7.0.2+ or amdgpu 30.10.2+
    local baseurl_amdgpu
    local baseurl_graphics
    if ([[ $DISTRO_MAJOR_VER -eq 8 ]] || [[ $DISTRO_MAJOR_VER -eq 10 ]]) && ([[ $ROCM_VERSION_LIBPATCH -ge 70002 ]] || [[ $AMDGPU_VERSION_LIBPATCH -ge 3001002 ]] ); then
        ...
        baseurl_amdgpu="https://repo.radeon.com/amdgpu/$mapped_amdgpu_version/el/$DISTRO_VER/main/x86_64/"
        baseurl_graphics="https://repo.radeon.com/graphics/$ROCM_VERSIONS/el/$DISTRO_VER/main/x86_64/"
        
        # adjust the baseurls for amazon linux
        if [[ "$DISTRO_NAME" = "amzn" ]]; then
            baseurl_amdgpu="https://repo.radeon.com/amdgpu/$AMDGPU_VERSION/amzn/$DISTRO_MAJOR_VER_AMZN/main/x86_64/"
            baseurl_graphics="https://repo.radeon.com/graphics/$ROCM_VERSIONS/amzn/$DISTRO_MAJOR_VER_AMZN/main/x86_64/"
        fi
    fi
    
    ## Register kernel-mode driver ##
    # amdgpu-lib in amdgpu repo for ROCm 6.4.2 and 6.4.3
