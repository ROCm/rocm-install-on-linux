...
1. ``rocm-cmake``
   ...
   For example, ``spack install rocm-cmake@7.1.1``
...
* ``spack spec`` command
  ...
  .. code-block:: shell-session
     ...
     -   mivisionx@7.1.1~add_tests~asan+hip~ipo build_system=cmake build_type=Release generator=make platform=linux os=ubuntu22.04 target=zen2 %cxx=gcc@11.4.0
   ...
   .. code-block:: yaml
      ...
      spack:
        ...
        - matrix:
          ...
          - [rocm-cmake@7.1.1, rocm-dbgapi@7.1.1, rocm-debug-agent@7.1.1, rocm-gdb@7.1.1,
            rocminfo@7.1.1, rocm-opencl@7.1.1, rocm-smi-lib@7.1.1, rocm-tensile@7.1.1, rocm-validation-suite@7.1.1,
            rocprim@7.1.1, rocprofiler-dev@7.1.1, rocrand@7.1.1, rocsolver@7.1.1, rocsparse@7.1.1,
            rocthrust@7.1.1, roctracer-dev@7.1.1]
        view: true
...
1. Stage the source code. For example:
   ...
   .. code-block:: shell
      ...
      spack stage hip@7.1.1
      # (This will pull the 7.1.1 release version source code of hip and display the path to spack-src directory where entire source code is available)

   You should see something like this:

   .. code-block:: shell-session

      ==> Using cached archive: /data/root/temp/rocm-7.1.1/spack/var/spack/cache/_source-cache/archive/d8/d8dba8cdf05463afb7879de2833983cafa6a006ba719815a35b96d9b92fc7fc4.tar.gz
      ==> Using cached archive: /data/root/temp/rocm-7.1.1/spack/var/spack/cache/_source-cache/archive/82/829e61a5c54d0c8325d02b0191c0c8254b5740e63b8bfdb05eec9e03d48f7d2c.tar.gz
      ==> Using cached archive: /data/root/temp/rocm-7.1.1/spack/var/spack/cache/_source-cache/archive/80/8081d4ab1a43ffa1cebd646668d83008b799ab98c14daf7b455922355a439c8a.tar.gz
      ==> Moving resource stage
              source: /tmp/root/spack-stage/resource-clr-zo53ondw3tevsr3gmoofbhre7asvis46/spack-src/
              destination: /tmp/root/spack-stage/spack-stage-hip-7.1.1-zo53ondw3tevsr3gmoofbhre7asvis46/spack-src/clr
      ==> Moving resource stage
              source: /tmp/root/spack-stage/resource-hip-tests-zo53ondw3tevsr3gmoofbhre7asvis46/spack-src/
              destination: /tmp/root/spack-stage/spack-stage-hip-7.1.1-zo53ondw3tevsr3gmoofbhre7asvis46/spack-src/hip-tests
      ==> Staged hip in /tmp/root/spack-stage/spack-stage-hip-7.1.1-zo53ondw3tevsr3gmoofbhre7asvis46

2. Change directory to ``spack-src`` inside the staged directory.

   .. code-block:: shell-session

      /spack$ cd /tmp/root/spack-stage/spack-stage-hip-7.1.1-zo53ondw3tevsr3gmoofbhre7asvis46
      /tmp/root/spack-stage/spack-stage-hip-7.1.1-zo53ondw3tevsr3gmoofbhre7asvis46$ cd spack-src/

3. Create a new Git repository.

   .. code-block:: shell

      /tmp/root/spack-stage/spack-stage-hip-7.1.1-zo53ondw3tevsr3gmoofbhre7asvis46/spack-src$ git init

4. Add the entire directory to the repository.

   .. code-block:: shell

      /tmp/root/spack-stage/spack-stage-hip-7.1.1-zo53ondw3tevsr3gmoofbhre7asvis46/spack-src$ git add .

5. Make the required changes to the source code.

   .. code-block:: shell

      /tmp/root/spack-stage/spack-stage-hip-7.1.1-zo53ondw3tevsr3gmoofbhre7asvis46/spack-src# vi hipamd/CMakeLists.txt
...
6. Generate the patch using the ``git diff`` command.
   ...
   .. code-block:: shell
      ...
      /tmp/root/spack-stage/spack-stage-hip-7.1.1-zo53ondw3tevsr3gmoofbhre7asvis46/spack-src$ git diff > /spack/var/spack/repos/builtin/packages/hip/0001-modifications.patch

7. Update the recipe with the patch file name and any conditions you want to apply.

   .. code-block:: shell

      /tmp/root/spack-stage/spack-stage-hip-7.1.1-zo53ondw3tevsr3gmoofbhre7asvis46/spack-src$ spack edit hip

8. Provide the patch file name and the conditions for the patch to be applied in the ``hip`` recipe as follows.

   .. code-block:: shell

      patch("0001-modifications.patch", when="@7.1.1")

   Spack will apply ``0001-modifications.patch`` on the 7.1.1 release code before starting the ``hip`` build.
