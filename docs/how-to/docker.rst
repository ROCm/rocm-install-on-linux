...
* ``amdgpu-dkms``: Docker containers share the kernel with the host OS. Therefore, the ROCm
  ...
  * `Check for amdgpu-dkms <https://instinct.docs.amd.com/projects/amdgpu-docs/en/docs-30.20.0/install/detailed-install/post-install.html#verify-kernel-mode-driver-installation>`_

  * If you don't have ``amdgpu-dkms``, follow the :ref:`standard ROCm installation instructions <rocm-install-quick>`
    (which comes with ``amdgpu-dkms``) or `install amdgpu-dkms separately <https://instinct.docs.amd.com/projects/amdgpu-docs/en/docs-30.20.0/install/package-manager-index.html>`__.
