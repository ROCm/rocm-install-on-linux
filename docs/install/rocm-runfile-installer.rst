...
.. code-block:: shell
   ...
   wget https://repo.radeon.com/rocm/installer/rocm-runfile-installer/rocm-rel-7.2.3/ubuntu/22.04/rocm-installer_1.2.8.70203-61-90~22.04.run
...
*  ``untar <directory> verbose``
   ...
   For example, if the ROCm 7.2.3 Runfile Installer is extracted using the ``untar`` command, the output directory is named ``rocm-7.2.3``.
   ...
   For example, for a ROCm 7.2.3 Runfile Installer ``untar`` extraction to the ``amd/myrocm`` user directory, use the folllowing commands to
...
*  ``postrocm``
   ...
   For example, if the current Runfile Installer is for ROCm 7.2.3, then
   ``rocm-install-path`` must indicate the path to a ROCm 7.2.3 Runfile installation.
   
   To use the ``postrocm`` argument separately from the initial install of ROCm 7.2.3
