...
- Add ROCm 7.2 support for Debian 12, 13, Ubuntu 22.04, 24.04, SUSE 15.7, RHEL 8.10, 9.4, 9.6, 9.7, 10.0, 10.1, Oracle Linux 8.10, 9.6, 10.0 and Rocky 9.6 and their ctests.
- Let users select kernel version when creating an offline installer package for driver installation on Oracle Linux.

### Changed

- Remove ctest `rocm-only-7.1` from the `ci` suite of tests and replace it with `rocm-only-7.1.1`. This creates the ROCm 7.1.1 installer package that the test `rocm-only-7.1.1-install` uses.
- Remove 7.1 and 7.0.2 from the `ci` suite of tests and replace them with 7.2 and 7.1.1.

## Offline Installer Creator v1.0.16

### Added

- Add ROCm 7.1.1 support for Debian 12, 13, Ubuntu 22.04, 24.04, SUSE 15.7, RHEL 8.10, 9.4, 9.6, 10.0, Oracle Linux 8.10, 9.6, 10.0 and Rocky 9.6 and their ctests.
- Add support for RHEL 9.7 and 10.1 for ROCm 7.1.1
- Map ROCm 7.1.1 to the 30.20.1 amdgpu driver
- Let users select kernel version when creating an offline installer package for driver installation on SLES.

### Fixed

- Fix issue in OL 9.6 where GDM (Gnome Display Manager) is being uninstalled during rocm installs for ROCm versions 6.4.1 to 7.1.1.

### Changed

- Switched to using color macros in the code
- Remove ctest `rocm-only-7.0.1` from the `ci` suite of tests and replace it with `rocm-only-7.1`. This creates the ROCm 7.1 installer package that the test `rocm-only-7.1-install` uses.
- Remove 7.0.1 and 7.0 from the `ci` suite of tests.

## Offline Installer Creator v1.0.15
