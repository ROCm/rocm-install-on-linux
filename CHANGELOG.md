...
## (Unreleased) Offline Installer Creator v1.0.9
...
- Remove ctest `rocm-only-6.3.3` from the `ci` suite of tests and replace it with `rocm-only-6.4`. This creates the ROCm 6.4 installer package that the test `rocm-only-6.4-install` uses.
- Update our installer test to `rocm-only-6.4-install`.
